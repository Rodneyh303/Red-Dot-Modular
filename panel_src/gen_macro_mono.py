...
def gen_macro(dark, W_MM=218.44):   # 43HP (42 + 1HP for the per-lane owner-source block)
    ...
    TAP_X=75.                          # P9: per-lane send PRE/POST tap (between attens & spread)
    L=[]; A=L.append
    A(D.svg_open(PW,PH))
    ...
    for row in range(4):
        ...
        # P9: one small PRE/POST send tap per lane, between the attens and the spread.
        A(f'<circle cx="{px(TAP_X):.1f}" cy="{px(y):.1f}" r="{px(2.2):.1f}" fill="{t["well"]}" stroke="{t["wellring"]}" stroke-width="0.9"/>')
    ...
    for row in range(4):
        ...
        # P9: ONE PRE/POST send tap per lane — placed between the atten cluster and the
        # spread knob. Named marker param_tap_{lane} bound to tapId(lane) in the widget.
        A(f'<circle id="param_tap_{lane}" cx="{px(TAP_X):.2f}" cy="{px(y):.2f}" r="0.5" fill="none" stroke="none"/>')
