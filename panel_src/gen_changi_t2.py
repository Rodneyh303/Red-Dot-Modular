#!/usr/bin/env python3
"""Changi T2 — per-voice STEP-GATE / STEP-LEGATO articulation output expander (20HP), airport diagram.

Second Changi terminal. T1 breaks out voice IDENTITY (gate/CV/accent); T2 breaks out the PHRASING
signals: per-voice STEP GATE (un-fused gate, every sub-note articulated) and STEP LEGATO (step gate
masked to slurs). Same airport-apron idiom as T1 (gold taxiways, runway centre-line, control tower,
aircraft glyphs at stands) — reused so the two terminals read as a matched pair, just two bands
instead of three.

16 voices (voice 0 = mono/ch0, voices 2..16 = poly), 2 jacks each: STEP GATE / STEP LEGATO.

nanosvg-safe (solid fills/strokes, elliptical-arc paths OK; no gradient/mask/text/url).

Kit id markers: output_{stepgate,steplegato}_<0..15> + light_connect
"""
import math
HP = 20
W  = HP * 5.08
...
THEMES = {
    "dark":  dict(bg="#0e120e", red="#d4001a", ink="#f0ead8",
                  ...
                  jackwell="#0a0c08", jackring="#5a4a1c", stepcol="#7cc48c", slegcol="#c48cd4",
    ...
    "light": dict(bg="#e8e4d4", red="#d4001a", ink="#2a2418",
                  ...
                  jackwell="#d8d0b8", jackring="#a89860", stepcol="#2c9a4c", slegcol="#9a4cb0",
                  paint="#9a8020", paintdim="#b0a068"),
}

MARGIN = 6.0
TITLE_H = 15.0
# Two bands: STEP GATE and STEP LEGATO. Taller than T1's 3 bands since we have more vertical budget.
BANDS = [("STEP GATE", "stepgate", 0, "stepcol"),
         ("STEP LEGATO", "steplegato", 1, "slegcol")]
BAND_TOP = 24.0
BAND_H   = 42.0
BAND_GAP = 4.0
FOOTER_Y = BAND_TOP + 2*(BAND_H+BAND_GAP)
...
def gen(dark):
    ...
    for bi in range(2):
        ...
        for side, sgn in ((0,-1),(1,1)):
            ...
            taxiway(A, t, x_run, by-9, x_out, by-6, curve=sgn*10)
            taxiway(A, t, x_run, by+9, x_out, by+6, curve=sgn*10)
            taxiway(A, t, x_out, by-6, x_out, by+6, curve=sgn*3, w=1.0)
    ...
    # ══ two jack bands (aircraft stands): 8 left | runway | 8 right ══
    JR = 3.1
...
def main():
    ...
    for dark, name in [(True, "ChangiT2_panel_dark.svg"), (False, "ChangiT2_panel_light.svg")]:
        with open(os.path.join(out, name), "w") as fh:
            fh.write(gen(dark))
        print(f"Changi T2 {'dark' if dark else 'light'}: res/panels/{name}  ({HP}HP, {PW}x{PH}px)")
