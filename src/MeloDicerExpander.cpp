...
struct MeloDicerExpander : Module {
    ...
    MeloDicerExpander() {
        ...
        for (int i = 0; i < 12; i++) {
            configInput(SEMI_CV_INPUT_0 + i, string::f("Semitone %d CV", i + 1));
        }
        configInput(OCT_LO_CV_INPUT, "Octave Lo CV");
        configInput(OCT_HI_CV_INPUT, "Octave Hi CV");
    ...
    void process(const ProcessArgs& args) override {
        ...
        if (rightExpander.module && rightExpander.module->model == modelMeloDicer) {
            ...
            for (int i = 0; i < 12; i++) {
                msg->semiCv[i] = inputs[SEMI_CV_INPUT_0 + i].getVoltage();
            }
            msg->octLoCv = inputs[OCT_LO_CV_INPUT].getVoltage();
            msg->octHiCv = inputs[OCT_HI_CV_INPUT].getVoltage();
...
struct MeloDicerExpanderWidget : ModuleWidget {
    MeloDicerExpanderWidget(MeloDicerExpander* module) {
        setModule(module);
        setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/MeloDicer_Expander.svg")));
        ...
        float padding = 5.0f;
        for (int i = 0; i < 6; i++) {
            addInput(createInputCentered<PJ301MPort>(mm2px(Vec(padding + i * 8.0f, 30.f)), module, MeloDicerExpander::SEMI_CV_INPUT_0 + i));
            addInput(createInputCentered<PJ301MPort>(mm2px(Vec(padding + i * 8.0f, 45.f)), module, MeloDicerExpander::SEMI_CV_INPUT_0 + 6 + i));
        }
        
        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(10.f, 70.f)), module, MeloDicerExpander::OCT_LO_CV_INPUT));
        addInput(createInputCentered<PJ301MPort>(mm2px(Vec(25.f, 70.f)), module, MeloDicerExpander::OCT_HI_CV_INPUT));
    }
};
