...
- **MPE_UTILITY_BUILD_SPEC.md** [Aug12, HIGH conf] -- **Keppel's three items BUILT (commit 60fd770) +
  code-reviewed 2026-09-03**: bend range now 1..48 (Keppel.cpp:71); re-articulation on range-exceed
  (default B, menu-toggleable, correct hysteresis -- re-notes on the same member channel, recentres so no
  boundary chatter); reverse-calc monitor OUTPUT (dotModular::mpe::reconstructVolts, MpeMath.hpp:66) wired
  + round-trip unit-tested (test_MpeMath.cpp, commit d1b331c: <1 cent at any range 1..48, analytic bound
  0.29c at +/-48). MpeMath split/inverse verified a true round-trip. Residual = Rack RUNTIME-verify only
  (scope MONITOR vs PITCH; real-DAW RPN handshake; full out->DAW->MPE-in round-trip) + panel marker
  `output_monitor` (Phase-2, guarded in the widget so it loads without it).
...
4. MPE_UTILITY_BUILD_SPEC -- Keppel range/re-articulation/monitor BUILT (60fd770) + reviewed + round-trip tested; Rack runtime-verify only.
